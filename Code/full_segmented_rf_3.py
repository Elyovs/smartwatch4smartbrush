...
for block_idx in range(len(chunk_boundaries)-1):
    ...
    chunk_features = scaler.transform(chunk_features)
    chunk_features = pca.transform(chunk_features)

    # Predict every window
    prob = model.predict_proba(chunk_features)
    vote = np.zeros(len(model.classes_))

    for p in prob:
        vote += p

    winner = np.argmax(vote)
    final_zone = model.classes_[winner]
    confidence = vote[winner] / np.sum(vote)

    print(f"\nBlock {block_idx+1}")
    print(dict(zip(model.classes_, vote.round(2))))
