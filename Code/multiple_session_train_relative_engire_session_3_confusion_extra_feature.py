...
def load_continuous_simulated_stream(file_list):
    ...
    for sess_id, paths in session_dict.items():
        ...
        for i in range(0, total_session_rows - window_size, step):
            ...
            # ==========================================
            # THE "SOFT GRAVITY" FIX IS HERE
            # ==========================================
            for col in sensor_cols:
                features.extend([
                    window[col].mean(), # <--- This gives the RF the absolute wrist angle
                    window[col].std(), 
                    window[col].max() - window[col].min()
                ])
                
            # Delta shifts (Notice how these also have means, but for the deltas, not the raw axis)
            for col in sensor_cols:
...
# Compute the Confusion Matrix for the Smoothed Predictions
...
plt.title("Confusion Matrix (RF Soft Gravity Fix)", fontsize=14, pad=20)
plt.tight_layout() 
