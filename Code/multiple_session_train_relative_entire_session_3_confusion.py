...
def split_files_by_random_sessions(file_list):
    ...
    print(f"   Training Sessions (3): {train_sessions}")
...
# 1. Compute the Confusion Matrix for the Smoothed Predictions
cm = confusion_matrix(y_test, smoothed_predictions, labels=model.classes_)

# 2. Create the display object
disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=model.classes_)

# 3. Plot the matrix
fig, ax = plt.subplots(figsize=(10, 8)) # Make the figure larger for readability
disp.plot(cmap=plt.cm.Blues, ax=ax, xticks_rotation='vertical') # Rotate x-axis labels 90 degrees

# 4. Add titles and layout adjustments
plt.title("Confusion Matrix (Smoothed Predictions)", fontsize=14, pad=20)
